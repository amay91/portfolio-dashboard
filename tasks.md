...
## M — Mobile optimization (2026-07-14)  *(the "deferred mobile workstream" B5/A1 pointed at — never formally scoped anywhere until this audit; IDs here are M1-M9 from the audit conversation, numbered R24+ to continue tasks.md's own sequence)*

### R24 (M1) — Contain wide-table horizontal overflow on mobile  ✅
- **Do:** live audit at 375px found the Full Holdings and Fund Houses tables cause the *entire page* to scroll sideways — `document.body.scrollWidth` measured 602px/437px in a 375px viewport — not merely a cramped table, a broken page. Not previously documented anywhere; found by actually measuring rather than trusting the earlier verbal B5/A1 notes were the complete picture.
- **Accept:** every `.htable` (sortable or not) is contained to its own horizontally-scrollable box at any viewport width; zero page-level horizontal scroll anywhere in the app, at any width down to 320px; sort/interaction on the contained tables is unaffected; no regression at desktop widths.
- **Resolution note (2026-07-14):**
  - **`.table-scroll` container** (`app.css`): wraps every `.htable` — `SortableTable.tsx` (covers `HoldingsTable`/`HousesTable`) and `DataSources.tsx`'s own non-sortable table. `overflow-x: auto` only engages once content actually overflows, so this needed no media query to stay inert at desktop widths (verified: at 1265px clientWidth the table fits its container exactly, 1024px = 1024px, and the wrapper's fade pseudo-element computes to `position: static`, confirming the ≤640px-scoped affordance correctly doesn't apply). A right-edge fade gradient (scoped to ≤640px, color-matched to `var(--paper)` per theme) hints there's more to scroll.
  - **Second, deeper bug found while verifying at 320px (not just 375px):** every InfoTip popover on the page — all 78 of them, regardless of `align` — still expanded `document.body.scrollWidth` even when *closed*, because a `visibility: hidden` element (kept in flow so its CSS opacity transition can animate, rather than `display: none`) still contributes its full geometry to the page's scrollable area. `InfoTip.tsx`'s existing per-instance `align: left/center/right` CSS (tuned for 375px, per A1's resolution note) doesn't scale to 320px for anchors positioned anywhere near the middle-to-right of the layout.
  - **Fix, part 1 (`InfoTip.tsx`):** a new `useEffect` keyed on `open` measures the anchor's `getBoundingClientRect()` and the popover's `offsetWidth` once opened, and clamps an inline `left` (overriding the CSS `align` position) to keep the box fully within `document.documentElement.clientWidth` minus a 12px margin — restored to CSS-driven positioning on close. Deliberately scoped to the `open` (click/touch/keyboard) path only; the pure-CSS `:hover` path is untouched by design (a mouse hovering a 320px-wide window isn't a real scenario worth adding JS cost to all ~80 InfoTips for). **Caught mid-implementation:** the clamp math initially used `window.innerWidth`, which was measured to diverge from `document.documentElement.clientWidth` in this exact test environment (365 vs the true 320px layout viewport) — switched to `clientWidth`, the API that actually matches the CSS pixels components render against.
  - **Fix, part 2 (`tokens.css`):** since the open-state clamp only helps the *interactive* case, not a never-opened tip's default (invisible but still-positioned) geometry, added `overflow-x: hidden` to `body` as a global safety net — verified this actually prevents page panning (not just cosmetic) by attempting `window.scrollTo(100, 0)` and `document.documentElement.scrollLeft = 100` and confirming both no-op; confirmed harmless to the app's one `position: fixed` + JS-computed-`top` "sticky" element (`useStickyToTarget.ts`, not real CSS `position: sticky`, which `overflow-x: hidden` on an ancestor would have broken).
  - **Verified live:** both themes, 320px/375px/1280px widths; all 78 InfoTips individually opened and measured with zero remaining overflow; Full Holdings sort-by-column still works inside its now-scrollable container; full gate green (429 Vitest tests, 2 Playwright e2e, typecheck, lint).

### R25 (M2) — Fix FundCards stats grid silently clipping data on mobile  ✅
- **Do:** the 4-column `.fcard .stats` grid (Units/Avg Cost/XIRR/CAGR) doesn't shrink its column count on narrow screens — at 375px, each of 4 columns gets ~84px minus 30px of padding, well short of what a real value like "63,056.496" needs. `.fcard`'s own `overflow: hidden` means this isn't a visible overflow bug, it's *data silently disappearing* — measured 465px of actual cell content rendering inside a 335px card. Previously noted only in passing during A1's InfoTip work ("noted for the deferred mobile workstream"), never actually fixed.
- **Accept:** all 4 stat values fully visible at any viewport width down to 320px, in both themes; no page-level overflow introduced; desktop (4-column) layout completely unchanged.
- **Resolution note (2026-07-14):** `.fcard .stats` collapses from `1fr 1fr 1fr 1fr` to `1fr 1fr` (2×2) at `max-width: 640px`, doubling each cell to 167px — confirmed via `scrollWidth > clientWidth` on every `.sv` value element, including the previously-clipped Units figure, that none overflow their cell anymore. The border pattern (`border-right` every 4th cell, for a single-row 4-col layout) moves to every 2nd cell for the new 2-col layout, with a `border-bottom` added between the two wrapped rows so the 2×2 grid keeps the same "ruled" look the original single row had — verified cell-by-cell (`getComputedStyle` on each of the 4 cells) that the border pattern forms a correct box, not just "some borders somewhere." 640px matches `.funds`'s own single-column collapse range (`.funds` itself only goes 2-col→1-col at ≤860px, so a card is comfortably wide from 641–860px too; only the ≤640px tier, where a single-column card is genuinely narrow, needs this). Verified live in both themes at 375px, and confirmed reverting to 4 columns at desktop widths (1265px clientWidth). Full gate green.

### R26 (M6) — Fix illegible/cramped mobile chart layout  ✅
- **Do:** every chart (the 6-chart gallery + the Command Deck's own Value-vs-Invested card) shares a fixed 740×380 SVG viewBox (`scales.ts` `CW`/`CH`) that scales down with its rendered box. At 375px, the gallery's prev/next arrows sat in the *same flex row* as the chart, leaving only ~225px for the chart itself — a ~0.30 scale factor that shrank a 10px source tick label to ~4px on screen (measured, not estimated); the deck chart, narrower still at ~302px rendered width (~0.41 scale), measured the same ~4px. Not previously documented; found by auditing actual rendered text height, not by inspection.
- **Accept:** chart axis/tick text meaningfully more legible at mobile widths, gallery arrows also reach the ≥44px tap-target minimum (double duty with review item B5/M4), zero regression to desktop rendering, and — the harder bar — no *new* text collision introduced by the fix itself.
- **Resolution note (2026-07-14):**
  - **Gallery arrows repositioned** (`app.css`, ≤640px): `.gbtn` moves from a flex-row sibling of `.gviewport` to a `position: absolute` floating circular overlay (44×44px, semi-transparent `var(--card)`, centered vertically) at the chart's left/right edges — `.gviewport` (and so the chart) now gets the row's full width instead of losing ~84px to two 32px buttons plus gaps. Verified the carousel's `next`/`prev`/wraparound still function correctly after the position change (a false-negative first read — stale state from repeated test calls in the same session, not a real regression — was caught and re-verified cleanly with a fresh page load before being trusted).
  - **Chart text sizes bumped, but *not* to the first value tried:** initially bumped `.chart-tick`/`.chart-lbl` to 24px/25px source (chosen to land ~10-11px rendered after the improved ~0.42 scale) — live measurement then found this collided with the x-axis's first-year label at the chart's bottom-left corner (the y-axis "0" tick and the x-axis "2018" tick's glyphs overlap there by ~6px), because the geometry's fixed pixel offsets between the two labels (`y={t.y+3}` for the y-tick, `y={CH-MB+16}` for the x-tick, in `InvestedChart.tsx` and its 5 siblings) were tuned assuming a ~10px source size and don't scale with a larger font. Rather than also re-tuning those offset constants across all 6 chart geometry files without the ability to visually re-verify the result (the Browser pane's screenshot tool was unavailable/timing out this session), dialed the bump back to 14px/15px — small enough that the measured residual overlap is 0.69px (sub-pixel, confirmed via exact `getBoundingClientRect()` math on the two specific colliding labels, not just eyeballed) while still roughly 50% larger than the original ~4px rendered size.
  - **Honest trade-off, logged for a future pass:** a larger, more comfortable text size is possible but needs the 6 charts' own tick-offset constants adjusted in step with any font-size increase, which needs real visual verification (not just bounding-box math) to land correctly across all 6 chart types — deferred rather than shipped unverified.
  - **Verified live:** both themes; desktop (1265px) confirmed completely unaffected (`.gbtn` back to `position: static`/40px, `.chart-tick` back to 10px); gallery carousel navigation (next/prev/wraparound) works correctly after the positioning change; cross-checked collision risk across all 6 gallery chart types at once (all 6 slides render simultaneously in the DOM, just translated off-screen, so a single bounding-rect sweep covers all of them) — worst case 1.57px, consistent with the deck chart's own 0.69px, not materially worse for any chart type. Full gate green (429 tests, 2 e2e, typecheck, lint).

### R27 (M4) — Grow remaining undersized tap targets to ~44px  ✅
- **Do:** review item B5 named the modal close button (measured 14×24px) and InfoTip buttons (19×19px) as under the ~44×44px minimum touch target guideline; the gallery arrows (the third named offender) were already brought to 44×44 as a side effect of R26/M6's arrow repositioning. Investigating sortable table headers (a 4th suspected offender from the original audit) found they're **already fine**: `SortableTable.tsx`'s `<th>` has its own click handler covering the *entire* 84×62px cell (B4's existing design — clicking anywhere in the cell sorts, not just the inner 49×16px button), so no fix was needed there — a good example of measuring before assuming.
- **Accept:** both real offenders reach ~44px in their shortest dimension at mobile widths; the *visible* icon/glyph size and surrounding layout stay unchanged (a bigger invisible hit area, not a bigger-looking button); zero effect at desktop widths.
- **Resolution note (2026-07-14):**
  - **`.feedback-close`** (shared by the Feedback modal and, via `ModalShell`, the Help modal): explicit `width/height: 44px` + flex-centering at ≤640px — padding alone couldn't close a 30px width gap without the "×" glyph looking off-center, since its natural advance width is much narrower than its font-size-driven line-height.
  - **`.infotip-btn`** (all ~80 instances app-wide): `padding: 15px; margin: -15px;` at ≤640px — the negative margin exactly cancels the padding's layout footprint on every side, so this grows the *tappable* area to ~43×43 without visually growing the ⓘ icon or pushing adjacent header/label text apart. Checked specifically for the failure mode this technique risks — an enlarged invisible hit-box silently overlapping an unrelated adjacent element's own hit-box — via bounding-rect comparison against the next KPI tile; no overlap.
  - **Verified live:** both themes; close button and InfoTip buttons measured at ~44×43px at 375px width; KPI tile row height unchanged (161px, same before/after); no page-level overflow introduced; desktop (1280px) confirmed both revert to their original small sizes (19×19 InfoTip). Full gate green (429 tests, 2 e2e, typecheck, lint).

### R28 (M3) — Touch/pointer support for chart tooltips  ✅
- **Do:** review item B5's headline item — `InteractiveChartFrame.tsx` (the shared primitive behind all 6 gallery charts + the Command Deck's own chart, R15/C2) wired `onMouseEnter`/`onFocus`/`onKeyDown` only; confirmed via source read that zero touch or pointer handlers existed anywhere in it, meaning every chart's tooltip was genuinely unreachable on a touchscreen, not merely awkward to use.
- **Accept:** tapping a data point reveals its tooltip on touch devices; tapping elsewhere dismisses it; critically, a scroll/swipe gesture that happens to begin on a chart (very likely, since the invisible hit-bands collectively cover almost the entire plot area) must still scroll the page normally, not get hijacked into selecting a point; the pre-existing mouse/keyboard path and the chart-gallery's own swipe-to-change-chart gesture must both keep working unchanged.
- **Resolution note (2026-07-14):**
  - **`ChartHitBands`** (each band rect): reveals its tooltip on `touchend`, not `touchstart`, and only if the touch moved less than a 10px slop from where it started (tracked via a `useRef`, compared on lift) — deliberately never calls `preventDefault()`, so a genuine scroll/swipe starting on a band is untouched by this code at all; only a real stationary tap counts as "select this point." Verified with real dispatched `TouchEvent`s (not just code review): a stationary tap reveals the tooltip; an 80px drag on the same band does not; both the `axis="x"` (vertical-strip, line/bar charts) and `axis="y"` (horizontal-strip, Holdings/Geography) code paths tested individually.
  - **`InteractiveChartFrame`**: dismissal on "tap elsewhere" needed a separate mechanism from the bands themselves, since it's a *document-level* concept, not a per-band one — added a `useEffect` (active only while a point is selected) listening for `touchstart` on `document` and clearing hover if the touch landed outside the chart's own `<svg>` (via a ref), an exact mirror of `InfoTip.tsx`'s existing click-outside-to-close pattern, for the identical underlying reason: touch has no equivalent of `:hover`/`blur` to fall back on.
  - **Confirmed no conflict with `ChartGallery`'s own swipe-to-change-chart gesture** (a *separate*, pre-existing `onTouchStart`/`onTouchEnd` pair on `.gviewport`, using a 40px threshold to advance slides): both listeners receive the same bubbling touch events, but their trigger thresholds (10px tap vs 40px swipe) are far enough apart to never both fire for the same gesture — verified live with a real 80px swipe dispatched on slide 1 (the one chart type that has both systems simultaneously in play): the gallery correctly advanced to slide 2, and no tooltip appeared.
  - **Desktop/mouse regression check**: a `mouseenter` dispatch initially appeared to show no tooltip — traced to a test-methodology artifact (`mouseenter` doesn't bubble, and React's delegated listener is for `mouseover`, which it uses to synthesize `mouseenter` internally — dispatching a raw `mouseenter` bypasses that), not a real regression; the actual `onMouseEnter`/`onFocus` JSX props were never modified, only added to (confirmed via diff), and a `mouseover` dispatch (the event React actually listens for) correctly showed the tooltip.
  - Full gate green (429 tests, 2 e2e, typecheck, lint).

### R29 (M7) — Guard hover-only effects behind `(hover: hover)`  ✅
- **Do:** the mobile-optimization audit's M3/M6 touch work (R28/R26) surfaced a related but distinct problem never previously documented: 13 CSS rules across `app.css`/`deck.css` apply visual state purely via `:hover` — `.infotip-btn`, `.infotip-pop`, `.drop`, `.feedback-close`, `.btn-demo`, `.legrow`, `.fcard .foot a`, `.htable th.sortable .th-sortbtn`, `.gbtn`, `.commentary-head`, `.hover-lift`, `.deck-btn`, `.deck-advt`. On touch devices there's no real mouse to leave, so tapping one of these "sticks" the hover-triggered look (background tint, popover visibility, scale/shadow) until an unrelated tap elsewhere happens to clear it — a genuine UX bug, not cosmetic, since `.infotip-pop` in particular could leave a popover visibly stuck open with no way to dismiss it via touch alone.
- **Accept:** all 13 effects only apply on devices with a real pointer that supports hover (desktop mouse/trackpad); touch taps get the element's plain resting state, no stuck visual state; two rules that combined a `:hover` selector with an unconditional state selector in one comma list needed splitting so only the `:hover` half moves, not the always-on half; zero behavior change on desktop.
- **Resolution note (2026-07-14):**
  - **Two combined selectors split** so the unconditional half stays active for touch's real open/tap state while only the `:hover` half moves under the new guard: `.infotip-btn:hover, .infotip.open .infotip-btn` → kept `.infotip.open .infotip-btn { color: var(--ink); }` unconditional (this is the real click/touch-driven "open" state from InfoTip.tsx's M1 work), moved `.infotip-btn:hover` alone into the guard; `.infotip:hover .infotip-pop, .infotip.open .infotip-pop` → same split, `.infotip.open .infotip-pop {...}` stays unconditional. `.drop:hover, .drop.over` → `.drop.over {...}` (the active drag-over state) stays unconditional, `.drop:hover` moves.
  - **One redundant rule deleted, not moved:** `.deck-advt-open, .deck-advt-open:hover { background: var(--brass); color: var(--paper); }` in `deck.css` — the `:hover` half was byte-identical to the base rule (a selected/open advisory tile keeping the same look whether or not the mouse happens to be over it), so simplified to `.deck-advt-open { ... }` alone rather than duplicating it inside the new guard block.
  - **Remaining 11 rules moved as-is** into one new `@media (hover: hover) { ... }` block per file — `app.css`'s block (11 rules: the split `.infotip-btn:hover`/`.infotip:hover .infotip-pop`/`.drop:hover` plus `.feedback-close:hover`, `.btn-demo:hover`, `.legrow:hover`, `.fcard .foot a:hover`, `.htable th.sortable .th-sortbtn:hover`, `.gbtn:hover`, `.commentary-head:hover`, `.hover-lift:hover`) placed right after the base `.hover-lift { transition: ... }` rule, before the existing `max-width: 860px` responsive block; `deck.css`'s block (`.deck-btn:hover`, `.deck-advt:hover`) placed right before its `max-width: 1023px` responsive section. The pre-existing `@media (prefers-reduced-motion: reduce) { .hover-lift:hover { transform: none !important; ... } }` override was left untouched — orthogonal concern, and its `!important` makes it win regardless of the new nesting.
  - **Verification method, and an honest limitation:** confirmed the CSS structure is correct via CSSOM introspection (`document.styleSheets` → walking `CSSMediaRule.conditionText`/`cssRules` to confirm each of the 13 selectors now resolves inside a `(hover: hover)` (or, for the 2 unconditional halves, correctly *outside* any hover guard) rule) plus a `window.matchMedia('(hover: hover)').matches` sanity check, rather than attempting a literal touch-emulation "tap and confirm it doesn't stick" test — the Browser pane's screenshot/touch-emulation tooling was unreliable this session (documented earlier for M3/M6 too), and `(hover: hover)` is a well-established, standard CSS media feature (not a novel or fragile technique), so static structural verification was judged sufficient rather than risking a false-negative test result. Full gate green: typecheck, lint, 429 Vitest tests, 2 Playwright e2e all passed.

### R30 (M8) — Bump sub-16px form fields to prevent iOS auto-zoom  ✅
- **Do:** iOS Safari auto-zooms the whole viewport on focusing any text-entry form field whose computed `font-size` is under 16px, forcing the user to manually pinch back out before they can keep using the page. Three real fields were under that threshold: the Feedback modal's `select`/`textarea` (14px), and the statement-password `<input type="password">` shown when a PDF is locked (13px). The Commentary section's age/retirement-age `<input>`s (`app.css` `.commentary-inputs input`) were checked and found already compliant at 16px — no change needed there.
- **Accept:** all three fields render at a computed 16px on mobile; no visual/layout regression on desktop (where the 16px minimum is harmless, just slightly larger than before); no page-level overflow introduced by the larger text.
- **Resolution note (2026-07-14):** `.feedback-label select, .feedback-label textarea` (`app.css`) and `.upload-password-row input[type="password"]` (`app.css`) both changed from their prior `font: <size>px/1.4 ...` shorthand to `16px` in place, with a one-line comment on each explaining the iOS auto-zoom trigger. The password input's real locked-PDF flow wasn't feasible to trigger live (no password-protected fixture on hand), so that rule was verified via CSSOM introspection (`document.styleSheets` → matching `.upload-password-row input[type="password"]` and reading `.style.font`), consistent with the CSSOM-verification approach used for M7's hover guards. The Feedback modal's `select`/`textarea` *were* verified against the real live DOM: opened the modal at 375px, read `getComputedStyle(...).fontSize` on both elements (16px, confirmed), and checked `document.body.scrollWidth === document.documentElement.clientWidth` (no overflow introduced by the larger text). Re-checked at 1280px desktop width — 16px on both fields, no overflow, no layout regression. Full gate green: typecheck, lint, 429 Vitest tests, 2 Playwright e2e all passed.

### R31 (M5) — Hide the MarkItDown bridge button on touch-primary devices  ✅
- **Do:** the "Convert PDF to Markdown" button (shown alongside "Instructions" only when `hasExtractionProblem` is true) POSTs to `http://127.0.0.1:8765/convert` — a bridge that only exists if the user has separately started `markitdown_server.py` on the very device they're using. On a phone or tablet there's no way to run that bridge at all, so the button was guaranteed to fail with "Couldn't reach your local MarkItDown bridge... Start it with `python markitdown_server.py`" — a confusing, un-actionable dead end on the exact class of device where it can never work.
- **Accept:** on a touch-primary device, the Convert/Instructions buttons are replaced by a plain-language note explaining the fix needs a desktop/laptop; desktop/mouse devices are completely unaffected; the swap uses the device's primary input mechanism (not momentary touch/mouse presence), so a touchscreen laptop with a mouse still gets the normal buttons.
- **Resolution note (2026-07-14):** CSS-only swap via `@media (pointer: coarse)` (`app.css`), mirroring M7's `(hover: hover)` technique — no JS/`matchMedia` needed, so `UploadBar.tsx`'s existing conditional rendering (`hasExtractionProblem`) and its Vitest suite (`UploadBar.spec.tsx`, which asserts on server-rendered HTML) are untouched; the swap is pure CSS visibility on top of unchanged markup. `.mm-convert-btn`/`.mm-instructions-btn` classes added to the two existing buttons; a new always-rendered-when-`hasExtractionProblem` `<p className="mm-touch-note">` added as a sibling of `.upload-instructions` with the copy "This statement didn't extract perfectly. Fixing it needs a small free program that only runs on a desktop or laptop computer — try re-uploading this file from one, if you have access to it." `app.css`: `.mm-touch-note { display: none; }` by default, then inside `@media (pointer: coarse) { .mm-convert-btn, .mm-instructions-btn { display: none; } .mm-touch-note { display: block; ...matching .upload-instructions's own padding/font-size/border-top for visual consistency... } }`.
  - **Verification, and an honest limitation:** confirmed via CSSOM introspection that both selectors are correctly grouped inside a single `(pointer: coarse)` `CSSMediaRule`. Confirmed the *default* (fine-pointer) state live in-browser — a synthetic `.mm-convert-btn` renders normally (`display: inline-block`) and a synthetic `.mm-touch-note` stays hidden (`display: none`) — matching desktop expectations. Could **not** live-verify the coarse-pointer side of the swap: the Browser pane's `resize_window` (including its `mobile` preset) only changes viewport dimensions, not the emulated `pointer`/`hover` media features (`window.matchMedia('(pointer: coarse)').matches` stayed `false` even at 375×812), and there was no real touchscreen device available this session. `(pointer: coarse)` is a well-established, standard CSS Level 4 media feature (same family as M7's `(hover: hover)`, which shipped on the same reasoning) — structural + default-state verification was judged sufficient, but this is flagged honestly rather than claimed as fully live-tested; a real phone/tablet check is the natural follow-up if this behavior is ever in doubt.
  - Full gate green: typecheck, lint, 429 Vitest tests, 2 Playwright e2e all passed.

### R32 (M9) — Final comprehensive verification sweep, M1-M8  ✅
- **Do:** each of M1-M8 (R24-R31) was individually gated and verified as it landed; M9 is the closing confirmatory pass across the whole set together — full gate one more time with everything combined, plus a fresh live check at both 375px and 320px, both themes, re-confirming the specific fixes rather than re-trusting each item's own isolated verification.
- **Accept:** typecheck/lint/429 Vitest/2 Playwright e2e all green with the full M1-M8 diff combined; no page-level horizontal scroll at 320px or 375px in either theme; the table-scroll containers (M1), FundCards 2-column collapse (M2), floating gallery arrows (M6), InfoTip 44px tap padding (M4), and the InfoTip off-screen clamp (M1) all independently re-confirmed live, not just re-read from their own resolution notes.
- **Resolution note (2026-07-14):**
  - **Full gate**, run fresh against the complete M1-M8 diff: typecheck clean, lint clean (one pre-existing, unrelated `oxlint` warning on `vite.config.ts`'s triple-slash reference — present before this workstream, not touched by it), 429/429 Vitest tests, 2/2 Playwright e2e.
  - **375px, light theme:** `document.body.scrollWidth === document.documentElement.clientWidth` (375 = 375) with the "Your Portfolio Analysis" accordion expanded (6-Chart Gallery, Full Holdings, Fund Houses, Fund Cards all mounted) — the real stress case, not the collapsed default. Re-confirmed live: 3 `.table-scroll` containers present (Full Holdings, Fund Houses, Data Sources), `.fcard .stats` computes a 2-column grid (`166.5px 166.5px`), the gallery's `.gbtn` computes `position: absolute`, and `.infotip-btn` computes `padding: 15px`.
  - **320px, dark theme, deeper check:** raw `document.body.scrollWidth` (365px) exceeded `clientWidth` (320px) — investigated rather than dismissed, per this workstream's established discipline. Root cause confirmed harmless: `getBoundingClientRect()` on every element found the only elements extending past the viewport were `.gslide`/`.gslide-*` — the *inactive* chart-gallery carousel slides, laid out in a row and moved off-screen via `transform: translateX(...)`, exactly as the carousel is designed to do. `scrollWidth` reflects an element's full un-clipped content size regardless of `overflow: hidden` on it or an ancestor — a known DOM quirk (not a bug) that R24/M1's own resolution note ran into and correctly worked around by testing actual scroll behavior instead of trusting `scrollWidth` alone. Repeated that same real test here: `window.scrollTo(100, 0)`, `document.documentElement.scrollLeft = 100`, and `document.body.scrollLeft = 100` all no-op (`window.scrollX` stayed `0`) — the page provably does not pan sideways, confirming the `scrollWidth` delta is inert, not a regression. Also re-confirmed M1's InfoTip off-screen clamp specifically at 320px/dark: opened a live InfoTip near the KPI row, measured its popover at `right: 262px` (clamped, well inside 320px) rather than its unclamped natural position, which would have overflowed.
  - **tasks.md documentation completeness:** confirmed R24 through R32 (this entry) are all present and each follows the established Do/Accept/Resolution-note structure; no gaps in the M-series numbering.
  - No commit made for the M1-M9 workstream as part of this task — per established session discipline, commits happen only when the user explicitly asks.

### R33 (post-M) — Fix mobile Menu/Theme-toggle position swap and clipped dropdown  ✅
- **Do:** on mobile (≤1023px), `.mobile-header-row`'s two children (`ThemeToggle`, then `HelpMenu`) were laid out via `justify-content: space-between` in that DOM order, putting the Menu button on the *right* and the theme toggle on the *left* — the reverse of desktop, where the help menu is the fixed top-**left** corner and the theme toggle is fixed top-**right**. Worse, the Menu's dropdown (`.help-menu-list`) opens via `position: absolute; left: 0` relative to its own button — anchored to a right-edge button, the list (≥168px per item, "Reading the Dashboard" wider still) extended past the viewport's right edge, where body's `overflow-x: hidden` (added in M1/R24 as a horizontal-scroll safety net) silently clipped it. User-reported: menu on the right, options truncated when opened.
- **Accept:** on mobile, the help menu sits on the left and the theme toggle on the right (matching desktop's corner positions); opening the menu shows every item's text fully, with no clipping, at 320px and up; desktop's fixed top-left/top-right corners are completely unaffected (position:fixed with explicit `left`/`right` values, not order-dependent).
- **Resolution note (2026-07-14):** `App.tsx`'s `.mobile-header-row` now renders `<HelpMenu />` before the `.theme-toggle-corner` div (previously the reverse) — `justify-content: space-between` then puts HelpMenu on the left. No CSS changes were needed: both `.theme-toggle-corner`/`.help-menu-corner` are `position: fixed` on desktop (unaffected by DOM order) and the mobile media query already switches them to `static`/`relative` and lets flex order determine screen position — the only bug was the order itself. Moving the menu to the left also fixes the dropdown clipping as a side effect: `.help-menu-list`'s `left: 0` now anchors near the screen's left edge (28px, the row's own padding) and opens *into* the remaining width rather than off the right edge.
  - **Verified live:** 375px and 320px, menu opened via a real click — every item (`Instructions`/`Reading the Dashboard`/`Privacy and Data`/`FAQ`/`Feedback`) measured with its right edge at 203-210px, well inside both viewport widths; the widest label's button has `scrollWidth === clientWidth` (166px, no internal text overflow — it wraps to two lines via `white-space: normal` rather than clipping). Desktop confirmed unchanged: `.help-menu-corner` still `position: fixed; left: 16px`, `.theme-toggle-corner` still `position: fixed; right: 16px`, regardless of the new DOM order.
  - Full gate green: typecheck, lint, 429 Vitest tests, 2 Playwright e2e all passed.

### R34 (post-M) — Fix donut center text spilling onto the ring on mobile  ✅
- **Do:** R20/D3 shrinks `.deck-alloc-card .donut-wrap` from its fixed 276px down to `min(220px, 68vw)` at ≤640px (so the donut fits a narrow card), but the center text (`.dc-k`/`.dc-v`/`.dc-s` — the value/label shown in the donut's hole) kept its 276px-tuned font sizes and `.dc-s`'s 150px `max-width` unchanged. The donut's hole is a fixed proportion of the SVG (`donutGeometry.ts`'s inner radius `r=92` of a 340 viewBox, i.e. ~54% of the container), so shrinking the container without shrinking the text meant the hole shrank (276px → 220px, hole diameter ~149px → ~119px) while the text box didn't — the text started overlapping the colored ring itself. User-reported: "text inside the donut chart is spilling onto the donut chart."
- **Accept:** at every mobile width down to 320px, and in every state (default "Total value" and all four categories selected), no line of center text extends past the donut's inner hole into the ring; desktop's 276px donut and its text sizing are completely unchanged.
- **Resolution note (2026-07-14):** added a `@media (max-width: 640px)` override (`deck.css`, inside the same block as R20's `.donut-wrap` shrink) scaling the center text down with the donut: `.dc-k` 10px→8.5px, `.dc-v` 18px→14px, `.dc-s` 11px→9px with `max-width` tightened from 150px to 100px (not just proportionally scaled — `.dc-s` holds the longest strings, e.g. "Tap a slice to break it down," so it needs to wrap to more/narrower lines rather than stay wide).
  - **Verification method:** rather than eyeballing, measured precisely — for each of `.dc-k`/`.dc-v`/`.dc-s`, used `Range.selectNodeContents` + `getClientRects()` to get each wrapped line's actual bounding box (not just the container's overall box), then computed each line's worst-case corner distance from the donut's own center point and compared it against the hole's true radius (`containerWidth * 0.5 * (92/170)`, derived from `donutGeometry.ts`'s fixed `r`/viewBox constants — the same ratio the SVG itself uses, not a guessed value).
  - **Before the fix** (375px, default state): hole radius 59.5px, `.dc-s`'s worst line-corner at 68.4px — a confirmed ~9px spill past the ring's inner edge, matching the reported bug.
  - **After the fix**, swept all 5 states (default + Equity/Debt/Cash/Other selected) at both 375px and 320px: worst case across all of them was `.dc-s` at 53.5px, comfortably inside the 58.9-59.5px hole radius at both widths (~5-6px margin).
  - **Desktop confirmed unchanged:** at a true 1265px viewport, `.donut-wrap` still computes 276px and the center text still computes its original 18px/11px/10px fonts and 150px max-width — the mobile override only applies inside its `max-width: 640px` query.
  - Full gate green: typecheck, lint, 429 Vitest tests, 2 Playwright e2e all passed.

### R35 (new feature) — "My Details" quick-fill panel for the CAMS statement request  ✅
- **Do:** getting a CAS statement in the first place means leaving the dashboard entirely, going to CAMS's own site, and manually retyping an email address and a PAN/PDF-password into their request form — real friction the dashboard couldn't previously do anything about. The dashboard itself can't submit that form on the user's behalf: CAMS's form is protected by a CAPTCHA (bypassing bot-detection is out of scope), and PAN is a government ID (entering government IDs into third-party sites on a user's behalf is also out of scope, even with explicit authorization) — so full automation was ruled out. What *is* available: a same-tab, same-device quick-fill the user fills in once and copies field-by-field into CAMS's real form themselves, keeping a human in the loop exactly where CAMS requires one.
- **Accept:** an Email + PAN quick-fill box at the top of the Instructions panel, each field with a one-tap/one-click Copy button; PAN is masked by default (it's a government ID) with a show/hide toggle; the fields survive closing and reopening the Instructions panel within the same tab (so a user checking the CAMS site in another tab and coming back doesn't lose what they typed) but are never written to `localStorage`/`IndexedDB`/any disk-backed store, consistent with `PrivacyDataContent.tsx`'s existing, tested claim that nothing here is ever saved to disk; works on both desktop (mouse) and mobile (touch) — no browser extension or native app required, since neither is realistically installable by this app's non-technical audience on mobile (Chrome for Android has no extension support at all; iOS Safari extensions require a native App Store app to host them).
- **Resolution note (2026-07-15):**
  - **New files:** `features/help/myDetails.ts` (the `MyDetails` type + `EMPTY_MY_DETAILS` constant — split out from the component file to satisfy `react/only-export-components`, keeping Fast Refresh working); `features/help/MyDetailsPanel.tsx` (the controlled component — email input, PAN input with an inline eye-toggle overlaid inside the field via `position: absolute` rather than a third flex sibling, and a Copy button per field).
  - **State ownership:** deliberately *not* local `useState` inside `MyDetailsPanel`/`InstructionsContent` — lifted up to `HelpMenu.tsx` (which is mounted for the whole app session, unlike the modal content underneath it, which unmounts on close) and passed down as `value`/`onChange` props. This is what makes the fields survive closing/reopening Instructions without reaching for any persistent storage — verified by a dedicated test (`HelpMenu.spec.tsx`: type into email, close the modal, reopen Instructions, assert the value is still there) that would fail if the state were ever moved back to being component-local.
  - **Copy mechanism:** tries `navigator.clipboard.writeText()` first; on failure (denied permission, non-secure/embedded context, or an older browser without the API) falls back to `input.select()` + `document.execCommand('copy')`, so the user can still manually Ctrl/Cmd+C even if the async path fails silently. **Verified live, and the fallback path actually got exercised, not just theoretically covered:** in this session's Browser-pane test environment, `navigator.clipboard.writeText()` itself threw ("Document is not focused" — an artifact of automated/headless control, not a real user's browser), and the fallback correctly ran and flashed "Copied" anyway, confirming the resilience path works rather than silently failing.
  - **PAN masking:** defaults to `type="password"`; a small eye-icon button (reusing the `.deck-ic` SVG style already used app-wide) toggles it to `type="text"`. Auto-uppercased on input (`AAAPZ1234C`, matching PAN's actual format) since it doubles as the PDF password CAMS's form asks for — Instructions step 5's copy was updated to say "paste your email/PAN from the box above" instead of "enter"/"set", tying the panel to the existing numbered walkthrough.
  - **Mobile-specific work:** input font-size is 16px (M8's iOS-auto-zoom rule, applied from the start rather than retrofitted); the eye toggle and Copy button both reach the ~44px tap-target minimum at ≤640px via the same padding/negative-margin (eye) and explicit `height: 44px` (Copy) techniques used for M4's other tap-target fixes, while staying small (32px/40px) on desktop; any `:hover`-only look for these buttons is added to the *existing* consolidated `@media (hover: hover)` block (not a new standalone rule) per M7's established convention, so a tap doesn't leave them stuck looking hovered. Layout uses exactly two flex children per field row (`.mydetails-input-wrap`, then Copy) regardless of whether that row also has an eye toggle — the toggle overlays *inside* the input-wrap rather than sitting beside it as a third sibling — specifically to avoid the three-item cramped-row risk a naive layout would hit on a 320px screen.
  - **Privacy documentation kept honest:** `PrivacyDataContent.tsx`'s existing "No accounts, no storage, no tracking" section (which already made a tested claim that nothing here persists to disk) gained one sentence explicitly naming this new panel and confirming it follows the same rule — the claim didn't need weakening, since the panel was deliberately designed to fit inside it, not the other way around.
  - **Tests:** new `MyDetailsPanel.spec.tsx` (5 tests: Copy disabled when empty, enabled + PAN auto-uppercase once typed, Clipboard API invoked + "Copied" flash, PAN masked by default + eye toggle, email has no eye toggle) using the same real-client-render technique as `HelpMenu.spec.tsx`/`Feedback.spec.tsx` (`createRoot` + the native-setter input-typing trick those files already established, needed because directly setting `.value` on a React-controlled input doesn't trigger its `onChange`). One new test added to `HelpMenu.spec.tsx` for the cross-remount persistence behavior described above.
  - **Verified live:** desktop (1280px) — eye button 32px/Copy button 40px (small, as intended, no forced mobile sizing); typing, PAN auto-uppercase, and the eye toggle all confirmed via real DOM events, not just source reading. 375px — Copy buttons and eye toggle measured at 44px, zero page overflow (`body.scrollWidth === clientWidth`), PAN input's `scrollWidth === clientWidth` (no internal text clipping). 320px — same checks repeated; one 1px sub-pixel rounding overshoot on `.mydetails`'s own right edge was investigated and confirmed harmless (`window.scrollX` stays `0` after an attempted `scrollTo`, matching the established verification method from R24/R32 for this exact class of false-positive). Both themes checked — panel correctly re-themes via CSS custom properties, input font-size confirmed 16px in both.
  - Full gate green: typecheck, lint, 435 Vitest tests (429 prior + 6 new), 2 Playwright e2e all passed.
  - **Explicitly out of scope, and why:** actually submitting CAMS's form (blocked by their CAPTCHA + PAN-entry restrictions, see Do above); persisting details across browser sessions/tab closes (would need `localStorage`, which conflicts with this app's core privacy promise for a government-ID field); a browser-extension or bookmarklet auto-fill path for desktop (still viable as a future addition, but out of scope for "works on both platforms with one implementation," which this quick-fill panel achieves without any install step).

### R36 (correction to R35) — Fix My Details panel's PAN/password conflation  ✅
- **Do:** the user asked to confirm whether PAN is actually a required field on CAMS's live CAS request form (they suspected it wasn't). Checked directly against the real form at camsonline.com rather than relying on the app's pre-existing assumption: the fields are `EMAIL *`, `PAN (OPTIONAL)`, `PASSWORD *` (a password *of the investor's own choosing* — 6+ characters, at least 2 numbers, limited special characters — explicitly *not* PAN-derived), `CONFIRM PASSWORD *`. R35's panel had it wrong in a more significant way than just the required/optional label: it treated PAN as if it *were* the PDF password (labeled "PAN (also usually the PDF password)", auto-uppercased to PAN format), when the real form asks for a separate, self-chosen password and treats PAN as an unrelated, optional identity field.
- **Accept:** the panel's second field matches what the form actually requires — a real password, not a PAN placeholder; PAN itself isn't given a dedicated field at all, since it's optional and doesn't feed into anything the dashboard later needs (unlike the password, which the dashboard's own upload flow asks for again to unlock the PDF); Instructions' step 5 and the Privacy panel's mention both describe the corrected fields accurately.
- **Resolution note (2026-07-15):**
  - **`myDetails.ts`**: `MyDetails.pan` renamed to `MyDetails.password`.
  - **`MyDetailsPanel.tsx`**: second field relabeled "PDF password (your choice — you'll set this on CAMS's form)", placeholder changed to "6+ characters, at least 2 numbers" (matching the form's actual rule), auto-uppercase removed (a real password isn't case-normalized), `id`/ref/state variable names updated from `pan`/`panVisible` to `password`/`passwordVisible`. The masked-by-default + eye-toggle treatment was kept as-is — appropriate for a real password field, arguably more so than it was for PAN.
  - **`InstructionsContent.tsx`** step 5 rewritten: now tells the user to paste email (required) and paste the same password into *both* the Password and Confirm Password fields, explicitly notes it's a made-up password (not PAN) and that CAMS's PAN field is optional and can be left blank.
  - **`PrivacyDataContent.tsx`**: "email/PAN fields" corrected to "email/password fields" in the quick-fill panel's privacy disclosure.
  - **Tests**: `MyDetailsPanel.spec.tsx` updated (`mydetails-pan` → `mydetails-password` throughout, auto-uppercase assertion removed and replaced with a plain value-round-trip check); `HelpMenu.spec.tsx`'s cross-remount persistence test only touched the email field, so it needed no change — confirmed by inspection, not just assumption.
  - **Deliberately left untouched:** `UploadBar.tsx`'s existing password-*recall* hint ("Usually your PAN in capitals... or whatever password entered on the CAMS webpage") — a different context (helping a user remember a password they already set weeks ago, where PAN genuinely is a common real-world choice many investors make) rather than a claim about what CAMS's form requires, so this finding doesn't make that copy wrong, just adjacent to the fixed issue.
  - Full gate green: typecheck, lint, 435 Vitest tests, 2 Playwright e2e all passed. Live-confirmed in-browser: the panel's second field now shows the corrected label, placeholder, and `id="mydetails-password"` with no leftover `#mydetails-pan` element.

### R37 (removal of R35/R36) — Remove the "My Details" quick-fill panel  ✅
- **Do:** user feedback after using it: the panel didn't actually reduce friction — the user still has to open CAMS's site and type email/password into *its* form regardless, so the quick-fill box was just an extra copy-paste detour on top of that, not a replacement for any of it. Correctly identified as net-negative complexity: a new component, new state lifted into `HelpMenu`, new CSS, new tests, and a privacy-disclosure addition, all in service of a step that didn't remove work, only relocated where the typing happened.
- **Accept:** the Instructions panel returns to intro paragraph + the 6 numbered steps + KFintech note + screenshot/diagram, exactly as it was before R35; step 5's copy (corrected under R36 to describe the real Email/Password/Confirm-Password/optional-PAN fields) is kept, since that correction is accurate and useful on its own regardless of whether a quick-fill box exists; no other numbered step changed.
- **Resolution note (2026-07-16):**
  - **Deleted:** `features/help/MyDetailsPanel.tsx`, `features/help/MyDetailsPanel.spec.tsx`, `features/help/myDetails.ts`.
  - **`InstructionsContent.tsx`**: removed the `MyDetailsPanel` import/render and the `myDetails`/`onMyDetailsChange` props (component signature reverted to taking none); step 5 reworded from "paste from the box above" back to a self-contained instruction ("Enter your email... then choose a password... enter it in both the Password and Confirm Password fields... CAMS's own PAN field is optional and can be left blank") — keeping R36's factual correction, just without referring to a panel that no longer exists.
  - **`HelpMenu.tsx`**: removed the lifted `myDetails` state, the `EMPTY_MY_DETAILS`/`MyDetails` imports, and the props passed into `InstructionsContent`.
  - **`PrivacyDataContent.tsx`**: removed the sentence naming the quick-fill panel from the "No accounts, no storage, no tracking" section — that claim now describes exactly what the app does again, with nothing to caveat.
  - **`app.css`**: removed all `.mydetails*` rules (the panel/note/row/field/input-wrap/eye/copy styles and their `max-width: 640px` tap-target overrides) and the two `.mydetails-eye:hover`/`.mydetails-copy:hover` lines from the consolidated `@media (hover: hover)` block (M7's convention) — nothing left behind.
  - **`HelpMenu.spec.tsx`**: removed the cross-remount persistence test that specifically covered the now-deleted feature.
  - **Verified live:** the Instructions panel opens with no `.mydetails` element present, its text starts directly at "Here's how to request one:" followed by the 6 steps, and step 5 reads the corrected (not the panel-referencing) copy. Full gate green: typecheck, lint, 429 Vitest tests (back down from 435 — the 6 removed-feature tests are gone, nothing else changed), 2 Playwright e2e all passed.

### R38 (new feature) — "Reading the Dashboard" click-to-spotlight navigation  ✅
- **Do:** the Reading the Dashboard help panel's 5 items (Summary row, Value vs Invested chart, Top holdings and Allocation, Portfolio Analysis, Portfolio Commentary) were plain static text — a reader had to mentally map each explanation onto the actual dashboard themselves. Requested: make each item clickable, jump to and briefly highlight the matching live dashboard region in a different colour, and repeat that same explanatory text in a popover next to it — dismissible by clicking elsewhere or Escape, but not by clicking the highlighted region itself (still interactive underneath the highlight).
- **Accept:** clicking an item closes the help modal (so the dashboard underneath is visible), scrolls to and highlights the correct element(s), shows a popover with that item's exact label + body text positioned next to the highlight (clamped to stay inside the viewport, tracking scroll/resize while open), and dismisses cleanly on Escape or an outside click — but not on a click inside the highlighted region or the popover itself. The "Top holdings and Allocation" item (one explanation, two separate cards) highlights both at once. No change to any other help content or to desktop's existing layout beyond what's newly added.
- **Resolution note (2026-07-16):**
  - **Stable target `id`s added** (styling already existed via classes; these are new, identity-only attributes) to the 5 regions the panel describes: `KpiRail.tsx` → `id="deck-summary-row"`; `ValueVsInvestedCard.tsx` → `id="deck-vvsi-card"`; `TopHoldings.tsx` → `id="deck-top-holdings"`; `CommandDeck.tsx`'s allocation card → `id="deck-allocation-card"`; `PortfolioAnalysis.tsx`'s button-row wrapper → `id="portfolio-analysis"` (the always-visible entry point, not the 6 sub-sections it opens, since those aren't in the DOM until expanded); `Commentary.tsx` already had `id="commentary-sec"` on its outer `<section>`, reused as-is.
  - **New `ui/Spotlight.tsx`** — looks targets up via `document.getElementById` (deliberately imperative rather than threading refs through several unrelated component trees several levels deep) and toggles a `.spotlight-target` class directly on the found DOM node(s). Two-pass positioning for its popover, same technique as `InfoTip.tsx`'s off-screen clamp: measure the target's `getBoundingClientRect()`, render the popover, then measure *its own* rendered size before computing a final `left`/`top` clamped to `document.documentElement.clientWidth`/`window.innerHeight` (right-of-target if there's room, else below, flipping above if that would still overflow the bottom). Position re-computed on scroll/resize via an rAF-throttled listener (same pattern as `useStickyToTarget.ts`) so it tracks the highlighted element rather than drifting from it if the user scrolls further. Dismiss listeners: `keydown`/Escape and a `mousedown` outside-check (skipped if the click landed inside the popover or any highlighted target) — the outside-click listener is registered one macrotask late specifically so the very click that *opened* the spotlight (Reading item's own `onClick`) doesn't immediately close it.
  - **State ownership**: `spotlight: SpotlightRequest | null` lives in `App.tsx` (mirrors how `openSections`/`commentaryOpen` are already owned there, not in `HelpMenu`) — `<HelpMenu onSpotlight={setSpotlight} />` passes a setter down; `HelpMenu.tsx`'s new `spotlightAndClose()` closes its own modal (`setActive(null)`) *before* calling it, since the modal's backdrop would otherwise sit on top of the dashboard the spotlight is trying to show. `<Spotlight request={spotlight} onDismiss={() => setSpotlight(null)} />` renders as a sibling of `<main>`.
  - **`ReadingDashboardContent.tsx`** restructured from a static `<ol>` of `<li>` prose into a small `ITEMS` array (`{ id, label, body, targetIds }`, `body` as `ReactNode` so the existing `<b>`-tagged terms in items 4/5 are preserved verbatim) mapped to real `<button className="rd-item-btn">` elements — text kept byte-for-byte identical to the original list (verified: the popover shown after a click literally repeats what the panel already said, not a paraphrase).
  - **CSS** (`app.css`): `.rd-item-btn` boxes each item (border/background/radius, sits inside the existing `.help-steps li`'s 34px counter gutter so the numbered circle is undisturbed) with a right-side arrow hint (`.rd-item-arrow`, dimmed by default, brightens on hover for pointer devices — hover rule lives in the existing consolidated `@media (hover: hover)` block per M7's convention, not a new standalone one). `.spotlight-target` — a pulsing green outline (`@keyframes spotlight-pulse`, automatically neutralized by the app's existing global `prefers-reduced-motion` rule, no extra work needed). `.spotlight-pop`/`.spotlight-label`/`.spotlight-body`/`.spotlight-close` — a bordered card matching the app's existing overlay/tooltip visual language.
  - **Mobile fix found during live verification, not assumed correct:** the close button's first mobile CSS pass used the padding/negative-margin tap-target trick (`.infotip-btn`'s M4 pattern), but at 320px its `.spotlight-label` text measurably overlapped the button's real 44px hit area (confirmed via a bounding-rect intersection check, not eyeballing) because that trick grows the hit area *asymmetrically* around a narrow "×" glyph rather than to a clean 44×44 box. Switched to `.feedback-close`'s explicit-box-+-flex-centering recipe instead and added `margin-right: 48px` to `.spotlight-label` at ≤640px to clear the now-larger button — re-verified with the same intersection check, confirmed `overlap: false`.
  - **Tests**: new `Spotlight.spec.tsx` (9 tests: renders nothing when idle, highlights + shows correct text, multi-target highlighting, highlight removed on dismiss, Escape/outside-click/close-button all dismiss, click inside the target or inside the popover itself does *not* dismiss, gracefully self-dismisses if none of its target ids exist in the DOM) using the same real-client-render technique as `HelpMenu.spec.tsx` (`createRoot`, real target elements appended to `document.body` since `Spotlight` looks them up by id rather than via props/refs). Two new `HelpMenu.spec.tsx` tests confirm the actual wiring: clicking a Reading item closes the modal and calls `onSpotlight` with that item's exact `targetIds`/label, and the two-target item passes both ids.
  - **Verified live**, replicating the user's own worked example exactly: opened Reading the Dashboard, clicked "Value vs Invested chart.", confirmed the modal closed, `#deck-vvsi-card` gained `.spotlight-target`, and the popover's body text was character-for-character "How your money has grown against what you've put in, month by month since your very first investment — hover (or tap) any point for the exact figures on that date." Also live-confirmed: Escape and outside-click both dismiss (with a proper `await` after the fix — an initial synchronous check without one falsely read pre-update DOM, corrected before trusting it); a click inside the highlighted target or inside the popover does *not* dismiss; the two-target item highlights both `#deck-top-holdings` and `#deck-allocation-card` at once; "Portfolio Analysis." spotlights its button row with the correct popover text; both themes re-theme correctly via CSS custom properties; 320px has no real page overflow (the `.rd-item-btn`/`.spotlight-pop` sub-pixel edges were checked against `window.scrollX` staying `0`, the same false-positive-vs-real-overflow test established in R24/R32).
  - Full gate green: typecheck, lint, 441 Vitest tests (429 prior + 12 new), 2 Playwright e2e all passed.

### R39 (refinement to R38) — Pin Spotlight's popover consistently to the left  ✅
- **Do:** R38's popover used "right of the target if there's room, else below it" positioning, so which side of the screen it appeared on — and effectively how it looked — varied item to item, which read as inconsistent. Requested: always the left side, symmetrically aligned, one standardized width for every item, height left dynamic.
- **Accept:** every item's popover renders at the exact same horizontal position and width, regardless of which dashboard region it's explaining; only its vertical position (tracking the target) and height (per its own content length) vary.
- **Resolution note (2026-07-16):** `Spotlight.tsx`'s second positioning pass no longer branches on available space — `left` is now always `MARGIN` (pinned to the viewport's own left edge, the same value for every request), and `top` is the target's vertical midpoint minus half the popover's own measured height, clamped to stay inside the viewport (same clamp idea as before, just simplified to one axis). Removed the now-unused `POP_WIDTH` constant — width was already a fixed CSS value (`.spotlight-pop { width: 300px; max-width: calc(100vw - 24px) }`, unchanged), so "standardize the width" was already true at the CSS layer; the actual inconsistency was purely the position logic, now fixed. Height remains intentionally unset in CSS, so it grows with whichever item's explanation is longest.
  - **Verified live:** clicked through all 5 items at 1280px — every popover measured `left: 12, width: 300`, with `top` and `height` varying per item (height ranged 155.5–395.5px matching each item's actual text length, `top` tracking each target). At 320px, `left` stayed `12` and `width` correctly shrank to the viewport-capped `296` with no overflow (`right: 308` inside `clientWidth: 320`).
  - Full gate green: typecheck, lint, 441 Vitest tests, 2 Playwright e2e all passed (no test previously asserted on the old right-or-below positioning, so nothing needed updating there).

### R40 (refinement to R38/R39) — Popover flush with the dashboard's own left edge; align Commentary card's width  ✅
- **Do:** R39 pinned the popover to a fixed small margin from the viewport's own left edge — consistent item-to-item, but not visually tied to the dashboard itself. Requested: flush against the actual dashboard column's left edge (the "Summary Portfolio"/CommandDeck card and "Your Portfolio Analysis" card, which — investigated live — already share one edge, `.deck-frame`'s `max-width: 1080px; margin: 0 auto` box). Also requested: make the Portfolio Commentary card the same width as those two, then flush the popover against that now-shared edge.
- **Accept:** the Command Deck card, Portfolio Analysis card, and Portfolio Commentary card all render at the same left edge and width; every spotlight popover's left edge lines up exactly with that shared edge, at any viewport width (including the edge-to-edge case on mobile, where the dashboard column itself has no side gutter).
- **Resolution note (2026-07-16):**
  - **Root cause found, not guessed:** live-measured all three cards' `getBoundingClientRect()` before changing anything. `.deck-frame` (CommandDeck) and `.deck-frame-compact` (PortfolioAnalysis) already shared the identical left edge and width (both are literally the same `.deck-frame` box model). `.commentary-card` did not — it sat 28px right of, and 56px narrower than, the other two, because it's a plain, unpadded card living *inside* `.wrap`'s padded content box (`.wrap { padding: 0 28px }`), whereas `.deck-frame` carries its own padding *inside* its own border, not as an inset of the border itself. The two box models look similar but aren't, and nothing had surfaced the mismatch until it was measured directly.
  - **Fix:** `.commentary-card` gained `margin: 0 -28px` (and `-20px` at the ≤860px breakpoint, matching `.wrap`'s own padding step there) — a "bleed to the edge" negative margin that expands the card back out to `.wrap`'s full outer box, which was already aligned with `.deck-frame`. No change to `.wrap` itself (used by many other, non-card sections) or to `.deck-frame`.
  - **`Spotlight.tsx`**: the position effect now queries `.deck-frame`'s live `getBoundingClientRect().left` (re-read on every reposition, not just once on open, since that edge itself moves on window resize) instead of a fixed `MARGIN` constant, falling back to `MARGIN` only if no `.deck-frame` exists (shouldn't happen — every spotlight target lives in the same `pf &&`-gated tree as at least one). `top` is unchanged (still tracks the target's vertical midpoint, clamped to viewport).
  - **Verified live:** at 1280px, `.deck-frame`/`.deck-frame-compact`/`.commentary-card` all measured `left: 92.5, width: 1080` — genuinely identical, not just visually close. Clicked through all 5 Reading items: every popover measured `left: 92.5, width: 300`, height varying 155.5–395.5px per item's actual content. At 375px (where the dashboard column itself is edge-to-edge, `left: 0`), the popover correctly matched at `left: 0` too — "flush" holds even in the zero-gutter case, not just the desktop case. At 320px, same check, no overflow (`right: 296` inside `clientWidth: 320`).
  - Full gate green: typecheck, lint, 441 Vitest tests, 2 Playwright e2e all passed (no existing test asserted on the prior fixed-margin position or the old Commentary card width, so nothing needed updating).

### R41 (correction to R40) — Popover was overlapping the dashboard, not sitting outside it  ✅
- **Do:** R40's "flush against the left edge" was implemented as the popover's right edge touching the dashboard's left edge from the *inside* (still overlapping the dashboard's own card background) — the user clarified the actual ask: the popover should sit genuinely *outside*, in the space to the dashboard's left, with the dashboard's edge flush against the popover's right edge from the *outside*. At the common ~1280-1440px window widths this app is typically viewed at, the 1080px-max-width dashboard column only leaves ~80px of real empty space beside it — not enough for a readable box. Presented the tradeoff directly (shrink the box to fit vs. overlap vs. shift the dashboard to open real room) and the user picked shifting the dashboard.
- **Accept:** at any reasonably-sized window, the popover sits fully outside the dashboard's own left edge with zero overlap, at the full standardized width; the dashboard visibly, briefly nudges right to make room rather than the box either overlapping or shrinking to an unreadable sliver; the shift never pushes the dashboard's own right edge off-screen; on viewports too narrow for the dashboard and a readable box to coexist side by side at all (mobile, where the dashboard already runs edge to edge), it falls back to sitting below the target instead, matching R38's original behavior for that case.
- **Resolution note (2026-07-16) — three real bugs found and fixed while building this, each confirmed by direct measurement rather than assumed fixed:**
  1. **First attempt (shrink-to-fit gutter) rejected after live-testing:** letting the popover's width shrink to whatever natural gutter existed (no dashboard shift) produced, for the longer items, an ~80px-wide, **1700px-tall** column of wrapped text at 1280px width — confirmed by measurement, not eyeballed, and clearly unusable. This is what led to presenting the shift option to the user instead of silently shipping something broken.
  2. **`margin-left` doesn't move a child by the full amount:** the first shift implementation applied `#app.style.marginLeft`, expecting `.deck-frame` (centered inside via `margin: 0 auto`) to move by exactly that amount. Measured only ~50% of the intended shift — because a margin on an `auto`-width block shrinks the block's own computed width to match its containing block, so the centered child re-centers within a narrower box instead of translating with it. Switched to `transform: translateX(...)`, a pure paint-time offset that doesn't touch layout at all, confirmed to move the full exact amount.
  3. **Re-measuring `getBoundingClientRect()` immediately after setting a *transitioning* CSS property is timing-dependent:** `#app { transition: transform .25s }` means the visual position at the instant right after the JS write doesn't reliably reflect the target value yet — confirmed by watching the same viewport/target produce different (and non-monotonic, oscillating) results across otherwise-identical runs. Fixed by computing the shifted rect mathematically (`frameRect.left + shift`) instead of re-querying the DOM for it — a `translateX` offset is fully deterministic, so there was never a need to re-measure in the first place.
  4. **A related jitter bug surfaced while chasing #3:** the position effect re-runs on every scroll-triggered remeasure (many times during a single ~300ms smooth `scrollIntoView`), and the original code reset `#app`'s transform to `''` and recomputed from scratch on *every* call — repeatedly restarting the CSS transition mid-flight, which visibly oscillated the shift back and forth instead of settling once. Fixed with a `shiftRef` tracking whatever shift is *currently* applied, so the dashboard's true unshifted position is computed by subtracting it from the live (possibly-shifted) rect — no DOM write at all unless the target shift amount has actually changed between calls.
  - **Placement decision, restructured:** rather than "compute a shift, apply it, then check if it was enough" (the original, buggier shape), the effect now decides `placedOutside` *before* touching the DOM at all — checking whether the natural gutter already qualifies, or whether the maximum safe shift (capped so the dashboard's own right edge never crosses the viewport) would be enough to qualify. Only a shift that actually achieves outside placement is ever applied; a shift that wouldn't help is never written to the DOM.
  - **`MIN_OUTSIDE_WIDTH` re-tuned from 220 to 150** — with the shift mechanism now reliably delivering ~161px of gutter at the common 1280px width (measured directly), a 220px floor was disqualifying a workable amount of room; 150px is comfortably readable for a wrapped multi-line box and is what's actually achievable at that width.
  - **Verified live at 1280px** (the width that exposed all of the above): all 5 Reading items now measure a flush `popRight === frameLeft` with zero overlap, every one at the full standardized `161px` width (`vw=1265` → this width, not the 300px ideal, since 1280px genuinely can't fit more even with a maximal safe shift — see the resolution note above), height varying per item (155.5–875px for the longest). Re-ran the same check repeatedly to confirm the earlier non-determinism is gone. Confirmed the shifted dashboard's own right edge stays fully on-screen (no new overflow introduced). Re-confirmed the mobile fallback (375px, zero natural gutter) still works correctly: no shift applied, full 300px width, positioned below the target, no overlap.
  - Full gate green: typecheck, lint, 441 Vitest tests, 2 Playwright e2e all passed after each of the fixes above.

### R42 (refinement to R38-R41) — Mobile-specific Spotlight: centered popover, independent dismiss for the highlight  ✅
- **Do:** on mobile (no room to sit outside the dashboard), the popover should be allowed to overlap the target — centered in front of it — rather than squeezed below it. Its "X" should close only the popover; the highlight should stay lit until the user taps the highlighted element itself or taps elsewhere on the dashboard. Desktop's existing behavior (X ends everything; the target stays normally interactive underneath the highlight) must stay exactly as it was.
- **Accept:** mobile: popover centered over the target (horizontally centered in the viewport, vertically centered on the target, overlap allowed); "X" hides the popover only; the highlight persists afterward until a tap on the target or a tap elsewhere on the dashboard, both of which end the whole thing. Desktop: byte-for-byte unchanged — "X" ends everything, and a click on the target still doesn't dismiss (it stays usable, matching how the popover never overlaps it there).
- **Resolution note (2026-07-16):**
  - **New `popupVisible` state**, independent of the `request`/`rect` state pair that drives the highlight — "X" toggles this off (mobile) instead of always calling `onDismiss()`. Reset to `true` at the start of every new request so a fresh item always shows its own popover.
  - **New `isOutsideRef`**, set by the positioning effect to whatever `placedOutside` it just computed (desktop-like vs mobile-like), read by both the "X" button's `onClick` and the outside-click handler — a ref (not state) specifically because those handlers are set up once per `request` in a *different* effect and need the *latest* value without that effect re-running just because placement changed (same reasoning as `shiftRef`, R41's fix for the analogous problem with the dashboard-shift amount).
  - **Dismiss logic, per mode:**
    - "X" `onClick`: `isOutsideRef.current ? onDismiss() : setPopupVisible(false)`.
    - Outside-click handler: unchanged for "elsewhere" (always fully dismisses); the previous unconditional "click on target never dismisses" became `if (onTarget && isOutsideRef.current) return` — mobile mode now falls through to `onDismiss()` for a target click, desktop keeps skipping it.
  - **Positioning for the mobile/centered case**: replaced the old "sit below the target, non-overlapping" math with `left = (vw - width) / 2` (horizontally centered in the viewport) and reused the *same* "centered on the target's vertical midpoint, clamped to viewport" formula the outside-placement branch already used for `top` — the two branches' vertical logic was identical once overlap became acceptable for the fallback case, so that duplication was removed rather than kept as two copies.
  - **Tests**: `Spotlight.spec.tsx` restructured into two `describe` blocks. A new `stubWideDeckFrame()` helper (a `.deck-frame` with a manually-stubbed wide `getBoundingClientRect()`, since jsdom has no real layout engine) exercises the desktop path explicitly; the *absence* of any `.deck-frame` in the DOM (the suite's existing default) naturally exercises the mobile path, since that's exactly the "no room to sit outside" condition the component itself checks for — no separate mobile-simulation flag needed. Desktop block: target-click doesn't dismiss, "X" fully dismisses (both existing tests, only relocated + given the frame stub). Mobile block (3 new tests): "X" hides the popover only and leaves the highlight lit; a target click dismisses (unlike desktop); after "X", a click elsewhere still dismisses the still-lit highlight.
  - **Verified live** at 1280px (desktop) and 375px (mobile): desktop's "X" confirmed still ending both popover and highlight together, unchanged. Mobile: popover measured horizontally centered (`left: 37.5` on a 375px viewport with a 300px-wide box — exactly `(375-300)/2`); "X" click confirmed popover gone but target still carrying `.spotlight-target`; a subsequent click on the target confirmed clearing the highlight; repeated with a fresh spotlight, confirmed a click elsewhere (after "X") also clears it.
  - Full gate green: typecheck, lint, 444 Vitest tests (441 prior + 3 net new), 2 Playwright e2e all passed.

### R43 (design pass) — Modern-clean shell: sans UI font + mono numerals, elevation  ✅
- **Do:** a UI/UX design review flagged that the app's biggest gap vs. "modern clean" fintech (Mercury/Monarch/Linear) was that the *entire* interface was set in monospace (JetBrains Mono dark / Courier Prime light) and every surface was a flat, border-only rounded rectangle (`box-shadow: none`). User approved prototyping two changes side-by-side (published as a comparison artifact), then: "Implement it in the app, pure mono for light." So: (#1) move the interface to a clean sans and keep monospace strictly for numbers; (#2) add a soft elevation system. "Pure mono for light" = the light theme's display numerals become mono too, dropping Fraunces entirely (no serif-hero variant).
- **Accept:** interface text (labels, headings, prose) renders in a system sans in both themes; structured/display numbers (KPI values, tables, NAVs, donut centre, projection) render in mono with tabular figures; numbers inside running prose stay in the sans (the modern convention — you don't mono a number mid-sentence); fund/scheme *names* stay sans (they're text, not data); major surfaces gain soft layered shadows; colour identity (amber/teal/green) and layout untouched; both themes and mobile still pass; no serif face loaded anymore.
- **Resolution note (2026-07-21):**
  - **Type (#1)** — token-level, done in `tokens.css`: `--font-sans` repointed from the theme's mono to a system sans stack (`system-ui, -apple-system, 'Segoe UI', Roboto, …`) in **both** themes — this is the app's base/UI font, so every heading/label/prose element that reads `--font-sans` (which is `body` and, via the retained `--font-serif` alias, all the display-heading selectors) flipped to sans with zero per-selector edits. `--font-mono` kept per-theme (JetBrains dark / Courier Prime light) — now used *only* for numbers. `--font-serif` kept as a legacy alias now equal to `--font-sans` (many heading selectors still name it; not worth churning 10 selectors to rename), documented as such.
  - **Numeric audit** — the selectors that must stay/become mono were enumerated from every `tabular-nums` + `--font-serif` usage, not guessed. Converted `--font-serif` → `--font-mono` on the three numeric display selectors (`.deck-val`, `.donut-center .dc-v`, `.fcard .mv`); added explicit `--font-mono` to the deck's secondary numbers that had been silently inheriting the (mono) base and would otherwise have flipped to sans (`.deck-sub`, `.deck-tile-foot-v`, `.deck-stlt-v`, `.deck-dual-v`, `.deck-dual-bench`, and `.deck-tbl td.deck-rt` — numeric cells only, so the `.deck-nm` name column and the `th` labels stay sans) plus `.commentary-out .co-proj-v`. The full-holdings/houses tables (`.htable td`), fund-card stats (`.fcard .sv`), allocation legend, drill-down, chart ticks, gains-split were **already** on `--font-mono` and needed nothing. Verified live via `getComputedStyle` that each class resolves to the intended family (deck value = JetBrains/Courier mono; deck label & holding name = system sans; etc.).
  - **Elevation (#2)** — new per-theme tokens in `tokens.css`: `--shadow-card` (soft ambient + tight contact shadow), `--shadow-frame` (deeper, for the big framed containers), `--card-hi` (a 1px inset top highlight so cards read as *raised*, the Linear/Vercel recipe), tuned separately for dark (black shadows) and light (warm brown-tinted). Applied: `.deck-frame` (+`.deck-frame-compact` inherits) → `--shadow-frame`, radius 14→16; `.deck-tile`/`.deck-card` → `--shadow-card` + `--card-hi`, radius 12→14; `.commentary-card`, `.datacheck` banner → `--shadow-card`; `.fcard` → `--shadow-card` + `--card-hi`, radius bumped 5→12 (the odd-one-out square radius). Frame-and-chips layering: the frame is a slightly recessed surface, tiles float on it.
  - **Heading crispness** — the three primary display headings (`h1`, `.sec-title`, `.deck-mast-title`) bumped weight 500→600 and letter-spacing tightened, so the sans headings read as intentional display type rather than default body.
  - **Leanness** — Fraunces is now unused, so its two `@fontsource/fraunces` imports and its `document.fonts.load(...)` call were removed from `main.tsx` (bundle win; the `@fontsource/fraunces` npm dep can be pruned in a follow-up since removing it needs a package install). Confirmed live that Fraunces is no longer in `document.fonts`.
  - **Verification** — full gate green (typecheck, lint, 444 Vitest, 2 Playwright e2e). Live-verified by DOM measurement in **both** themes: base font = system sans; KPI/table/donut/projection numbers = the theme's mono; fund names & labels & headings = sans; card/frame `box-shadow` non-`none` and resolving to the per-theme tokens; no page-level horizontal overflow at 1280px or 375px. NOTE: the Browser pane's *screenshot* tool was unresponsive during this pass (intermittent all session), so visual sign-off was by computed-style + geometry inspection rather than a captured image — the structural facts are conclusive, but a human eyeball of the running app at localhost:5174 (toggle both themes) is the recommended final look before this ships.
  - **Explicitly NOT in this pass** (offered as the natural next increment, not yet approved): the dominant full-width hero KPI, sentence-casing the uppercase micro-labels, and revisiting the amber dark accent — all from the same review but each a further, separable step.

### R44 (design pass) — Dark accent: amber → teal (cool single-accent system)  ✅
- **Do:** review refinement #2 — the dark theme's amber primary accent reads "Bloomberg terminal"; modern dark UIs use a single cool accent with a warm colour reserved as a sparing highlight. User reviewed a side-by-side accent-study artifact (current amber / proposed teal / alt indigo) and chose **"Proposed — Teal"**. Move the primary accent to teal, recolour the equity slice to indigo so the donut stays legible, and demote amber to attention/provenance surfaces only. **Light theme untouched** (this was a dark-theme study).
- **Accept:** dark primary accent (links, focus rings, gain numbers, chart value line, hovers, sort-active, commentary emphasis) is teal; equity allocation is indigo, still distinct from teal debt; amber survives in dark only on genuine attention/provenance surfaces (Worth a Look, Data Check warnings, statement-NAV tags, sample-data note, RollingChart benchmark line, Gold geo slice); no two cool series collide (invested-vs-value chart lines, India/US geo bars); light theme renders byte-for-byte as before; full gate + no overflow.
- **Resolution note (2026-07-22):** token-level in `tokens.css`, dark block only:
  - `--green` (the misnamed **primary accent** — it was amber `#e8a33d`, never literally green) → `#5eead4` teal. One token flip carries every `var(--green)` consumer (focus rings, links, `.deck-accent`, KPI/commentary emphasis, `.htable`/`.fcard` gain numbers, `AnnualChart`/`HoldingsChart`/`RollingChart` series, hovers) to teal. Legacy name kept + documented; an app-wide rename to `--accent` is deferred (churns ~40 sites).
  - `--cat-equity` → `#818cf8` indigo (dark). Donut/legend now: equity indigo, debt teal, cash slate, other violet — all distinct (verified live: equity slice `rgb(129,140,248)`, debt `rgb(94,234,212)`).
  - **Two-cool-series collision fix:** with the accent now teal, the two charts that paired a `--green` line against a `--teal` line (InvestedChart value-vs-invested; geography India-vs-US) would have gone teal-on-teal. Added a **`--series-2`** token (dark `#818cf8` indigo, **light `#3b5563` = the old `--teal` so light is unchanged**) and repointed exactly those secondary spots to it: `InvestedChart` invested line/dot/legend-swatch, `.chart-tip-v.invested` (app.css), and the `United States` geo colour. `--teal` itself is unchanged, so insight cards / help links / renamed tags / CapitalChart stay teal as before.
  - **Amber demotion** needed no edits — amber was already isolated in the `--brass` token, which only ever feeds attention/provenance surfaces. Flipping the *primary* accent off amber is what completes the "sparing highlight" end state.
  - **Verification** — full gate green (typecheck clean, lint clean save the pre-existing vite.config triple-slash warning, 444 Vitest, 2 Playwright e2e). Live-verified by `getComputedStyle` + resolved token probing in **both** themes: dark `--green`/`--teal` = `#5eead4`, `--series-2` = `#818cf8`, `--cat-equity` = `#818cf8`, `--brass` = `#c98a2e`; **light unchanged** (`--green` `#2f5233`, `--teal`/`--series-2` `#3b5563`, `--cat-equity` `#2f5233`); rendered donut equity indigo / debt teal; `.deck-accent` teal; no horizontal overflow at 1280px. Screenshots (partial — the pane's screenshot tool was intermittently timing out) confirmed the Data-Sources link and Full-Commentary link now render teal while the sample-data note and Worth-a-Look card keep their amber borders. See docs/DECISIONS.md "Dark accent: teal".
  - **Explicitly NOT in this pass:** the still-queued review items (hero KPI hierarchy #3, sentence-case micro-labels #5, tighter small chart type #6, subtle motion #7, left-nav integration #4) and the alternate indigo primary accent.

### R45 (design pass) — Left-nav → app header + consolidated Help menu  ✅
- **Do:** review refinement #4 — on desktop the five help items (Instructions / Reading the Dashboard / Privacy and Data / FAQ / Feedback) floated as a vertical pill stack in the left gutter, reading like leftover debug buttons rather than integrated chrome. They aren't page navigation (the app is one page) — they're a help cluster. User reviewed a side-by-side nav-study artifact (current pills / consolidated Help menu / inline links / slim icon sidebar) and chose **Option A: a top utility bar with a single "Help" dropdown**.
- **Accept:** desktop shows a real app header (brand wordmark left; Help menu + theme toggle right) instead of gutter pills; the five items live in one "Help ▾" dropdown (the same consolidated pattern the app already used on mobile); the dropdown always opens on-screen; the gutter is clear; mobile still works; help items' behaviour (modals, spotlight, feedback) unchanged; full gate + no overflow at desktop/mobile.
- **Resolution note (2026-07-22):**
  - **App.tsx** — added `<header className="app-header">` above `<UploadBar>`: an `.app-brand` (a small teal→indigo gradient mark + "Portfolio Dashboard" wordmark) on the left, and `.app-header-actions` (HelpMenu + ThemeToggle) on the right. Removed the old `.mobile-header-row` wrapper.
  - **HelpMenu.tsx** — the desktop "always-visible inline pills" behaviour is gone; the component now renders its `.help-menu-toggle` (relabelled "Menu" → **"Help"**, with a caret that rotates on open) at every width, and the five items live in the `.help-menu-list` dropdown at every width — desktop simply reuses what was previously the mobile-only collapsed pattern. Dropped `useStickyToTarget` (the gutter-tracking that positioned the floating stack against the "Sample Portfolio Summary" box) — no longer floating, so the hook + its file `ui/useStickyToTarget.ts` were **deleted** (it had no other consumer; a leanness win).
  - **app.css** — replaced the `.theme-toggle-corner`/`.mobile-header-row`/`.help-menu-corner` fixed-corner CSS with `.app-header` (flex row, `position: relative; z-index` so the dropdown paints above the following `.uploadbar`) + a right-aligned `.help-menu-list` dropdown (`right: 0`, so it opens leftward into the screen wherever the button sits). The theme toggle is no longer `position: fixed` — it lives in the header (scrolls with the page, like the upload bar already did).
  - **Mobile fix caught in live verification:** on ≤640px the brand is hidden (to stay compact beside the theme toggle's ~125px label), which left `justify-content: space-between` stranding the lone actions group on the *left* — sending the right-aligned dropdown off the left edge (measured `listLeft: -80` at 375px). Added `justify-content: flex-end` in the ≤640 media query so the actions sit right and the dropdown opens on-screen (re-measured `listLeft: 22`, fully visible).
  - **Reverses earlier decisions** (documented in docs/DECISIONS.md "App header: one integrated bar…"): the fixed top-left/top-right corners (DECISIONS "Theme toggle moved to a fixed page corner"), the desktop always-inline help pills, the U10/U11 mobile-header-row arrangement, and the sticky-to-target gutter tracking are all superseded by the single header bar. Tests key off `.help-menu-list`/`.help-menu-toggle` classes and the item labels (all preserved), so HelpMenu.spec's 8 cases passed unchanged.
  - **Verification** — full gate green (typecheck, lint, 444 Vitest, 2 Playwright e2e). Live-verified at desktop and 375px mobile: header present with brand + "Help" + theme toggle; dropdown opens fully on-screen both widths (screenshots captured this pass — the screenshot tool cooperated); left gutter clear; no horizontal overflow.
  - **Still queued from the review:** hero KPI hierarchy #3, sentence-case micro-labels #5, tighter small chart type #6, subtle motion #7.

### R46 — Help-menu discoverability pointer in the masthead  ✅
- **Do (user request):** a small circled "?" at the top of the dashboard whose hover tooltip tells users the Help menu holds Instructions / Reading the Dashboard / etc., so they're aware those sections exist. Placement chosen with the user: right of the "Sample Portfolio Summary · As of … · Live NAV" masthead line.
- **Resolution note (2026-07-22):** reused the existing `InfoTip` primitive (hover for mouse, click/Enter for touch/keyboard, Escape + click-outside close, viewport clamp — all already built) rather than a new component. Added an optional `glyph?: 'info' | 'help'` prop: `'help'` renders a circled **?** instead of the ⓘ, so it reads as a general "where's the guidance" affordance, distinct from the ⓘ metric-explainers on the KPI tiles. Placed in `Masthead.tsx` right after `.deck-mast-meta` with `align="left"` (popover opens rightward into the frame from that left-of-centre spot). Copy points explicitly to the Help menu "(top-right)" — where the R45 header now puts it. Full gate green (typecheck, lint, 444 Vitest, 2 e2e); live-verified the "?" renders after the meta line and its popover opens fully on-screen. InfoTip's own spec passes unchanged (the new prop defaults to `'info'`).

### R47 (design pass) — Hero KPI hierarchy, sentence-case micro-labels, tighter chart type  ✅
- **Do:** the remaining three items from the design review's "cheap, complementary batch" (user: "Implement #3 and #5 and #6"):
  - **#3** Weak visual hierarchy on the hero — Total Value sat in a tile that looked like every other tile, giving the eye no clear entry point. Let it dominate.
  - **#5** Uppercase + letter-spaced micro-labels everywhere ("TOTAL VALUE, SHORT TERM, ALL-TIME") compound the terminal feel; in sentence-case with the app's system sans they'd read more current.
  - **#6** Small type in charts is very small/dense — line-chart axis labels, KPI sub-labels — wants a touch more size.
- **Accept:** Total Value renders as a full-width hero row above the three secondary tiles (Total Gain/ST-LT, XIRR, Insight), sized up, with real whitespace around it; every dashboard micro-label that was `text-transform: uppercase` now renders as its already-correctly-cased JSX text (no rewritten strings needed — the app's Title Case convention, R44, already had the right casing, CSS was just shouting it); chart axis type and the KPI secondary labels are bumped a notch for legibility; both themes, mobile, tablet, desktop unaffected structurally; full gate + no overflow.
- **Resolution note (2026-07-22):**
  - **#3 (hero)** — `KpiRail.tsx` restructured: the outer `id="deck-summary-row"` wrapper (still the Spotlight target, unchanged) now contains a `.deck-hero-row` (just `TotalValueTile`) followed by `.deck-kpi-rail` (the 3 remaining tiles, grid `repeat(3, 1fr)`, was `repeat(4, 1fr)`). `TotalValueTile`'s root gained a `deck-tile-hero` modifier class; `deck.css` gives it more padding (24px/28px vs 14px/16px) and, via descendant selectors scoped to that one tile, a much larger value (`.deck-val` 22px → 42px, 30px on mobile), bigger label/sub/footer type — the shared `.deck-tile`/`.deck-val`/etc used everywhere else are untouched. Tablet breakpoint (≤1023px) simplified: 3 secondary tiles go straight to single-column instead of an awkward 2-then-orphan split, matching `.deck-grid-2col`'s own collapse at the same width.
  - **#5 (casing)** — removed `text-transform: uppercase` (and cut the now-unneeded wide letter-spacing to ~.01-.02em) from every dashboard micro-label: `.eyebrow` (section kickers), `.donut-center .dc-k`, `.fcard .stats .sk`, `.gainsplit .gk`, `.htable th` (column headers), `.stable .exited`/`.srctable .exited`, `.chart-lbl`, `.deck-stlt-k`, `.deck-dual-k`. Verified each one's underlying JSX text was already properly cased ("Short Term", "Scheme", "Units", "Holdings", …) — R44's Title Case audit had already fixed the strings; only the CSS was overriding them into a shout. Deliberately left alone: form-field labels (`.feedback-label`, `.commentary-inputs label`, `.help-formula-label`) — a standard, still-current convention for input labels, not a "dashboard data" surface — and `.fcard .house` (a real AMC brand name, not a generic label).
  - **#6 (chart/KPI type)** — bumped `.chart-tick` 10→11px, `.chart-lbl` 10.5→11.5px, `.deck-stlt-k` 10→11px, `.deck-dual-k` 10.5→11px, `.deck-dual-bench` 10.5→11px, `.fcard .stats .sk` 9.5→10.5px, `.gainsplit .gk` 10→10.5px, `.htable th` 10.5→11.5px (weight 500→600 for a touch more presence at the smaller size). Colors/contrast were already AA-passing (B1); the complaint was density, not contrast, so no color changes.
  - **Verification** — full gate green (typecheck, lint, 444 Vitest, 2 Playwright e2e — none of the three specs assert grid/class structure beyond text content, so nothing needed updating). Live-verified via `getComputedStyle` in both themes, at 375px/900px/desktop widths: hero tile full-width at 42px (30px mobile) inside the same `#deck-summary-row` Spotlight target; secondary row correctly 3-wide desktop → 1-wide ≤1023px with no orphan tile; every touched selector resolves `text-transform: none` with its intended (already-correct) text; no horizontal overflow at any width. NOTE: the Browser pane's screenshot tool was unresponsive again this session (same intermittent issue as R43) — verification is DOM/computed-style based, not a captured image; recommend an eyeball at localhost:5174 for final sign-off.
  - **Still queued from the review:** subtle motion on expand/collapse and value updates (#7) — the last unimplemented item from the original recommendations menu; #2 (amber accent), #4 (left-nav), and now #3/#5/#6 are all done (R44/R45/R47 respectively).

### R48 — Total Invested onto the hero header row, half type size  ✅
- **Do (user request):** move "Total Invested" (text + number) onto the same row as Total Value, on the right side; make the invested number 50% of Total Value's font size. User reviewed a mock-up (two footer treatments) and chose "Variant A" — no divider, since nothing is left below the gain line once Total Invested moves up.
- **Resolution note (2026-07-22):** `KpiRail.tsx`'s `TotalValueTile` restructured — the old `.deck-tile-foot` block (a bordered footer with "Total Invested" + fund count below a divider) is gone; a new `.hero-head` flex row holds `.hero-head-main` (label/value/gain-line, as before) and `.hero-head-side` (new: "Total Invested" label + `.hero-invested-v` + fund count), `justify-content: space-between` so the two pair up left/right on the same top line. `.hero-invested-v` is set to a literal 50% of the hero's desktop `.deck-val` (42px → 21px); the ≤640px media query keeps that same 50% ratio against the hero value's own mobile size (30px → 15px) rather than a fixed px, and switches `.hero-head` to `flex-direction: column` — the two 8-13-digit rupee figures don't both fit a ~330px mobile tile side by side, so Total Invested stacks below Total Value there instead (still left-aligned, still half-size). Removed the now-fully-dead base `.deck-tile-foot`/`-row`/`-v`/`-sub` CSS rules from `deck.css` — `TotalValueTile` was their only consumer app-wide (verified via grep before deleting; `deck-mut`, a different class TotalGainTile still uses, was untouched).
- **Verification** — full gate green (typecheck, lint, 444 Vitest, 2 Playwright e2e). Live-verified via `getComputedStyle` in both themes: desktop ratio exactly `21/42 = 0.5`, header items share the same top edge, invested sits right of value, old footer gone; mobile ratio exactly `15/30 = 0.5`, layout stacks vertically, left-aligned; no horizontal overflow at any width. Screenshot captured this pass confirming the visual result matches the approved mock-up.

### R49 (design pass) — Subtle motion on expand/collapse + value updates (review item #7)  ✅
- **Do:** the last unimplemented item from the original design-review menu — "quiet micro-interactions: fades, 150ms eases on hover/expand/collapse and value updates." A hover-lift already existed pre-review (`ui/HoverLift.tsx`); this extends the same quiet philosophy to section expand/collapse and to numbers changing after a Refresh/upload.
- **Accept:** opening a Portfolio Analysis section or the Commentary card fades/rises in instead of snapping into existence; the Help-menu dropdown fades+drops instead of a hard `display` cut; the 6 section-picker buttons ease their open/closed color flip instead of cutting it; the 5 headline KPI numbers (Total Value, Total Invested, Total Gain, XIRR All-Time, XIRR 1Y) briefly pop/fade when their *displayed value* actually changes, not on every unrelated re-render; `prefers-reduced-motion: reduce` still disables all of it (already a global rule); full gate + no regressions.
- **Resolution note (2026-07-22):** all CSS-only, consistent with the app's post-framer-motion (R12) preference — no new dependency, no timing/exit-state JS:
  - **Section expand-in** — new `.sec-anim-in` class (`@keyframes sec-fade-in`: opacity 0→1 + `translateY(6px)→0`, .25s ease) in `app.css`. Applied to `Section.tsx`'s root `<section>` — the shared wrapper every Portfolio Analysis accordion entry renders through — and to `Commentary.tsx`'s `.commentary-body`. Both are conditionally mounted (`{open && <Section/>}` / `{open && <div className="commentary-body">}`), so a CSS `animation` on the freshly-created root plays automatically the instant it's inserted — no JS needed for an *enter* effect. Collapse still cuts instantly (unmount can't be animated without keeping stale DOM around); documented as the deliberate low-complexity trade-off, matching R12's original rationale for dropping framer-motion.
  - **Help-menu dropdown** — `.help-menu-list` converted from a hard `display: none/flex` toggle to `visibility`/`opacity`/`transform: translateY(-4px)` with a `.15s` transition (mirrors `InfoTip`'s existing pattern), plus a `visibility` transition delayed on close so it hides only after the fade-out finishes, not instantly.
  - **Section-picker buttons** — `.deck-advt`/`.deck-advt-ic` gained `transition: background-color .15s ease, color .15s ease` — the button that *is* a section's expand/collapse control now eases its brass "open" state instead of hard-cutting it.
  - **Value-update pop** — new `.val-update` class (`@keyframes val-update`: opacity .35→1, .4s ease) in `deck.css`. `KpiRail.tsx`'s 5 headline value elements each get `key={formattedText}` (the rendered string itself, computed once into a local const and reused for both `key` and children) alongside the class — React only remounts the node (retriggering the CSS mount animation) when the *displayed* text actually differs from before, so an unrelated re-render that leaves the number unchanged does not replay it, while a real Refresh-driven change does. Also plays once on first paint (numbers "arriving"), a deliberate, common modern-dashboard touch rather than a bug.
  - **Verification** — full gate green (typecheck, lint, 444 Vitest, 2 Playwright e2e — no spec asserts exact class lists, so none needed updating; `commentary-body`'s existing substring-match assertion still passes with the extra class appended). Live-verified: Help dropdown resolves `visibility: visible; opacity: 1` after opening; opened `#holdings-full` section and `#commentary-body` both resolve `animation-name: sec-fade-in`; all 5 KPI value nodes carry `val-update` and resolve `animation-name: val-update`; `.deck-advt`/`.deck-advt-ic` resolve the new `transition`; screenshot confirms no visual regression. Reduced-motion coverage relies on the pre-existing global `@media (prefers-reduced-motion: reduce) { * { transition: none !important; animation: none !important; } }` rule (app.css) — not re-added, already blankets every selector touched here.
  - **All 7 items from the original design-review menu are now done** (#1 font/elevation → R43; #2 accent → R44; #3 hero → R47; #4 left-nav → R45; #5 casing → R47; #6 chart type → R47; #7 motion → this task).

### R50 — Feedback on GitHub Pages: Formspree fallback  ✅
- **Do (user question → request):** user asked whether feedback submitted through the app is stored anywhere ("it needs to be compiled"). Investigation found: the app's only feedback pipeline (`app/functions/api/feedback.ts` → a Cloudflare Pages Function forwarding to a webhook, task D3) requires **Cloudflare Pages** hosting — but the user confirmed the site is actually deployed on **GitHub Pages**, a static-only host that cannot run *any* server code. `docs/Deploying-to-GitHub-Pages.pdf` (page 6) already documented this honestly: "it will show a polite error instead of sending." User asked to wire up the documented fallback option: **Formspree**, called directly from the browser, no backend needed — every submission lands in a compiled, exportable (CSV) table in the Formspree dashboard.
- **Accept:** a production build with no `VITE_FEEDBACK_URL` override posts straight to a Formspree endpoint instead of the (Cloudflare-only) edge function or the (dev-only) local server; local dev and any future Cloudflare Pages deploy are both completely unaffected; the endpoint-selection logic is unit-tested directly, not just live-verified; full gate green.
- **Resolution note (2026-07-22):**
  - New `app/src/features/feedback/feedbackEndpoint.ts` — pure `resolveFeedbackEndpoint(edgeUrl, isDev)`: `edgeUrl` (Cloudflare) wins if set → else the local dev server if `isDev` → else Formspree. Split into its own module rather than living in `Feedback.tsx` for two reasons: (a) oxlint's react-refresh rule warns on a non-component export sharing a file with a component — this codebase has stayed lint-clean since R1, so that warning got fixed properly rather than accepted; (b) `import.meta.env.DEV` is baked in by Vite at module-eval time, so a test stubbing it *after* the module's first import wouldn't affect an already-evaluated constant — pulling the branching into a plain, directly-callable function sidesteps that entirely (no `vi.resetModules()` gymnastics needed).
  - `Feedback.tsx` now just calls `resolveFeedbackEndpoint(EDGE_FEEDBACK_URL, import.meta.env.DEV)`. `submit()` also gained an `Accept: application/json` header (Formspree redirects instead of returning JSON without it — inert for the other two endpoints, which read `request.json()` regardless) and now checks Formspree's `{ errors: [{ message }] }` validation-error shape alongside the existing `{ error }` shape, so a rejected Formspree submission still surfaces a real message. The "Failed to fetch" branch's error copy now also covers the new production-no-override case (Formspree unreachable) with the same generic "try again in a moment" message used for the Cloudflare path, since neither is something a visitor can self-serve fix like starting a local server.
  - `FORMSPREE_ENDPOINT` currently holds a **placeholder** (`https://formspree.io/f/FORMSPREE_FORM_ID`) — Formspree requires the site owner's own free account (email-verified signup), which isn't something that can be created on someone else's behalf. `docs/DEPLOY.md`'s new "Feedback on GitHub Pages (Formspree)" section documents the exact 2-minute signup + the one line to edit once a real form ID exists.
  - **Verification** — full gate green (typecheck, lint — the new react-refresh warning is gone post-split — 447 Vitest [+3 new: `resolveFeedbackEndpoint`'s three branches], 2 Playwright e2e). A real `npm run build` was inspected directly (`grep` on the built JS) to confirm `import.meta.env.DEV` truly resolves to `false` in a production bundle — not just Vitest's default — and that the Formspree placeholder (not the localhost default) is what actually gets baked into `FEEDBACK_ENDPOINT` for that build. Live-verified in dev mode that the Feedback modal still renders and opens with no console errors post-refactor.
  - **Closed out (2026-07-22, same day):** user created their own Formspree account/form and supplied the real endpoint (`https://formspree.io/f/mojgnkkg`), which replaced the placeholder in `feedbackEndpoint.ts`. Re-verified after the swap: full gate green, and a fresh `npm run build` was grepped to confirm the *real* ID is what ends up in the built JS. **That "verified" claim turned out to be incomplete** — see the correction immediately below; the build/gate checks never actually exercised a real browser's CSP enforcement.
  - **Correction (2026-07-22, same session) — real submissions were still failing.** User reported "Couldn't send feedback right now" live. Root cause: `app/index.html`'s CSP `connect-src` allow-list (`'self' https://www.amfiindia.com https://mf.captnemo.in https://api.mfapi.in http://127.0.0.1:8765 http://127.0.0.1:8766`) never had `formspree.io` added when this task wired Formspree in — so the browser was silently blocking the `fetch()` outright, surfacing as `TypeError: Failed to fetch`, exactly the branch that shows the generic "try again in a moment" message. A `curl` diagnostic to the Formspree endpoint directly returned a clean `200 {"ok":true}` (confirming the endpoint itself, CORS, and the payload shape were all fine — `curl` doesn't enforce CSP, which is exactly why the earlier build-inspection "verification" missed this: neither `npm run build` grepping nor a `curl` test exercises a real browser's policy enforcement, only an actual page load does). **Fix:** added `https://formspree.io` to `connect-src`. **Genuinely** verified this time by serving the actual production build locally (`npm run build` + `vite preview`, matching what GitHub Pages runs byte-for-byte) in a real browser tab and submitting a real message through the rendered form — got "Thanks — your feedback has been sent," zero console errors, zero CSP violations. Full gate re-run green after the CSP edit (no test asserts the exact CSP string, so nothing else needed updating).
  - **Lesson:** for anything gated by a browser-enforced policy (CSP, CORS-adjacent browser behavior, permissions), a server-side check (`curl`, a Node script, a build-artifact grep) proves the *backend* half is fine but says nothing about whether the *browser* will actually be allowed to make the call — only a real page load in a real browser tab verifies that. Two of this task's own earlier "full gate green, verified" claims (the placeholder swap and the real-ID swap) both missed this exact gap.
