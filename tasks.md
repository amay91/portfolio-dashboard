...
- **Resolution note (2026-07-22):**
  ...
  - **Closed out (2026-07-22, same day):** user created their own Formspree account/form and supplied the real endpoint (`https://formspree.io/f/mojgnkkg`), which replaced the placeholder in `feedbackEndpoint.ts`. Re-verified after the swap: full gate green (typecheck, lint, 447 Vitest, 2 Playwright e2e), and — same as the placeholder check — a fresh `npm run build` was grepped directly to confirm the *real* ID (not the old placeholder, not the localhost default) is what actually ends up in the built JS. Feedback on the live GitHub Pages site now genuinely delivers to the user's own compiled, exportable Formspree inbox.
